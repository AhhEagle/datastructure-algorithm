...
function findSum(arr, value) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    if (index != false && i != index) return [arr[i], value - arr[i]];
  }
  return false;
}
...
function binarySearch(arr, item) {
  ...
  while (first <= last && !elementFound) {
    ...
    if (arr[mid] == item) {
      ...
      if (item < arr[mid]) last = mid - 1;
      else first = mid + 1;
    }
  }
  if (elementFound) return index;
  else return false;
}
...
function findFirstUnique(arr) {
  ...
  for (let index = 0; index < arr.length; index++) {
    ...
    while (index1 < arr.length) {
      if (index != index1 && arr[index] == arr[index1]) break;
      index1 += 1;
    }
    if (index1 == arr.length) return arr[index];
  }
  return null;
}
